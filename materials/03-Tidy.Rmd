...
bp_systolic2 %>% 
  group_by(subject_id) %>% 
  summarise(
    average = mean(systolic),
    last_measurment1 = last(time),
    last_measurment2 = max(time)
  ) 

bp_systolic %>% 
  pivot_longer(2:4, names_to = "time", values_to = "count") %>% 
  group_by(subject_id) %>% 
  summarise(
  average = mean(count)
 )
# or
bp_systolic %>% 
  pivot_longer(cols = c("time_1", "time_2", "time_3"), names_to = "time", values_to = "count", names_prefix = "time_") %>% # names prefix is a built regex to remove the beginning of words 
  group_by(subject_id) %>% 
  summarise(
  average = mean(count)
 )
# or
bp_systolic %>% 
  pivot_longer(cols = -subject_id, names_to = "time", values_to = "count") %>% 
  group_by(subject_id) %>% 
  summarise(
  average = mean(count)
 )
...
table4a %>% 
  pivot_longer(cols = 2:3, names_to = "year", values_to = "cases", names_ptypes = list(year = integer())) # convert year into an integer  
```
